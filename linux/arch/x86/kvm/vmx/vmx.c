...
{
	struct vcpu_vmx *vmx = to_vmx(vcpu);
/*My code, variable declaration Vishal*/
	int startTime;
	int endTime;
	long time;
	int returnVal;
	...
	{
		returnVal = handle_invalid_guest_state(vcpu);		
		endTime=rdtsc();
	...
	{
		returnVal = nested_vmx_reflect_vmexit(vcpu, exit_reason); 	
		endTime=rdtsc();
		time= endTime - startTime;
		atomic_long_add(time, &totalTimeSpent); 
		atomic_long_add(time, &eixtNumTimeStampArray[exit_reason]); 
	...
	{
		returnVal = kvm_vmx_exit_handlers[exit_reason](vcpu);
		endTime=rdtsc();
		time= endTime - startTime;
		atomic_long_add(time, &totalTimeSpent); 
		atomic_long_add(time, &eixtNumTimeStampArray[exit_reason]); 
